...
const AdminSetup = () => {
	...
	const stripeStatusRequestedRef = useRef(false);
	const youtubeStatusRequestedRef = useRef(false);
	...
	const applySettings = useCallback((nextSettings: IntegrationSettings) => {
		setSettings(nextSettings);
		setYoutubeCredentials({
			clientId: nextSettings.youtube?.clientId || '',
			clientSecret: nextSettings.youtube?.clientSecret || '',
		});
	}, []);
	...
	// Fetch the shared settings once. External integration status calls are lazy per tab.
	useEffect(() => {
		if (!canAccessPayments || settingsRequestedRef.current) return;
		settingsRequestedRef.current = true;
		void fetchSettings();
	}, [canAccessPayments, fetchSettings]);

	// Open the relevant tab after an OAuth redirect. The active-tab effect performs one status fetch.
	useEffect(() => {
		...
		if (youtubeConnect) {
			...
			if (youtubeConnect === 'success') {
				...
				setError(urlParams.get('error') || 'Failed to connect YouTube account');
			}
			...
			if (stripeConnect === 'success') {
				...
				setError(urlParams.get('error') || 'Failed to complete Stripe Connect onboarding');
			}
			window.history.replaceState({}, '', window.location.pathname);
		}
	}, []);
	...
	useEffect(() => {
		...
		if (activeTab === 'stripe' && !stripeStatusRequestedRef.current) {
			stripeStatusRequestedRef.current = true;
			void fetchStripeConnectStatus();
		}

		if (activeTab === 'youtube' && !youtubeStatusRequestedRef.current) {
			youtubeStatusRequestedRef.current = true;
			void fetchYouTubeStatus();
		}
	}, [activeTab, canAccessPayments, fetchStripeConnectStatus, fetchYouTubeStatus]);
		...
		try {
			...
			if (activeTab === 'zoom' || Object.values(settings.zoom).some((v) => v && !isMasked(v))) {
				payload.zoom = {
					...
					meetingSdkSecret:
						settings.zoom.meetingSdkSecret && !isMasked(settings.zoom.meetingSdkSecret) ? settings.zoom.meetingSdkSecret : settings.zoom.meetingSdkSecret,
				};
			}
			...
			if (hasSmtpChanges) {
				payload.smtp = {
					...
					fromAddresses: {
						...
						marketing: settings.smtp.fromAddresses?.marketing && !isMasked(settings.smtp.fromAddresses.marketing) ? settings.smtp.fromAddresses.marketing : '',
						noreply: settings.smtp.fromAddresses?.noreply && !isMasked(settings.smtp.fromAddresses.noreply) ? settings.smtp.fromAddresses.noreply : '',
					},
				};
			}
			...
			const { data } = await axios.put('/integration-settings', payload);
			if (data.settings) {
				applySettings(data.settings);
			}
			setSuccess(true);
	...
	return (
		<DashboardPagesLayout pageName="Account Setup" customSettings={{ justifyContent: 'flex-start' }} showCopyRight={true}>
					...
					>
						...
						{activeTab === 'smtp' && (
							<Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
								...
								>
									<CustomTextField label="SMTP User" value={settings.smtp.user} onChange={(e) => handleInputChange('smtp', 'user', e.target.value)} fullWidth />
									<CustomTextField
										label="SMTP Password"
										type="password"
								...
								<Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 1 }}>
									<CustomTextField
										...
										fullWidth
									/>
									<Tooltip title="e.g., noreply@mycompany.com (for automated emails: payments, notifications, bug report confirmations)" arrow placement="top">
										<IconButton size="small" sx={{ color: 'text.secondary', mt: '0.25rem' }}>
											<InfoOutlined fontSize="small" sx={{ fontSize: '1rem' }} />
										</IconButton>
									</Tooltip>
						...
						{activeTab === 'youtube' && (
							<Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
								...
								<Typography variant="body2" sx={{ mb: 2, color: 'text.secondary' }}>
									Connect your YouTube account to automatically upload Zoom meeting recordings. Recordings will be uploaded as unlisted videos and displayed in
									your app.
								</Typography>
			...
			{/* Zoom Help Dialog */}
			{zoomHelpDialogOpen && (
				<Dialog open={true} onClose={() => setZoomHelpDialogOpen(false)} maxWidth="md" fullWidth>
					...
					<DialogContent>
						...
						<Typography variant="body2" sx={{ mb: 2, color: 'text.secondary' }}>
							When setting up your <strong>Server-to-Server OAuth</strong> app, make sure to enable the following scopes in the <strong>Scopes</strong> tab:
						</Typography>
						<List dense>
							<ListItem>
