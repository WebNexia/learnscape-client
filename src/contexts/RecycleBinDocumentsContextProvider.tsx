...
interface RecycleBinDocumentsContextTypes {
  ...
  setArchivedDocuments: React.Dispatch<
    React.SetStateAction<ArchivedDocument[]>
  >;
...
export const RecycleBinDocumentsContext =
  createContext<RecycleBinDocumentsContextTypes>({
    archivedDocuments: [],
    totalItems: 0,
    currentPage: 1,
    searchResults: [],
...
export const RecycleBinDocumentsProvider = ({
  children,
}: RecycleBinDocumentsContextProviderProps) => {
    ...
    async (page: number = 1) => {
      ...
      try {
        const response = await axios.get(
          `${base_url}/documents/organisation/${orgId}/archived?page=${page}&limit=200`,
        );
  ...
  return (
    <RecycleBinDocumentsContext.Provider value={contextValue}>
      {children}
    </RecycleBinDocumentsContext.Provider>
  );
};
...
export const useRecycleBinDocuments = () => {
  ...
  if (!context) {
    throw new Error(
      "useRecycleBinDocuments must be used within a RecycleBinDocumentsProvider",
    );
  }
  return context;
};
