...
interface RecycleBinQuestionsContextTypes {
  ...
  setArchivedQuestions: React.Dispatch<
    React.SetStateAction<ArchivedQuestion[]>
  >;
...
export const RecycleBinQuestionsContext =
  createContext<RecycleBinQuestionsContextTypes>({
    archivedQuestions: [],
    totalItems: 0,
    currentPage: 1,
    searchResults: [],
...
export const RecycleBinQuestionsProvider = ({
  children,
}: RecycleBinQuestionsContextProviderProps) => {
  ...
  const [searchResultsTotalItems, setSearchResultsTotalItems] =
    useState<number>(0);
  ...
  const fetchArchivedQuestions = useCallback(
    async (page: number = 1) => {
      if (!orgId) return;

      try {
        const response = await axios.get(
          `${base_url}/questions/organisation/${orgId}/archived?page=${page}&limit=200`,
        );
  ...
  return (
    <RecycleBinQuestionsContext.Provider value={contextValue}>
      {children}
    </RecycleBinQuestionsContext.Provider>
  );
};
...
export const useRecycleBinQuestions = () => {
  ...
  if (!context) {
    throw new Error(
      "useRecycleBinQuestions must be used within a RecycleBinQuestionsProvider",
    );
  }
  return context;
};
