...
export const READING_SKILL_IDS = [
  "main_idea",
  "detail",
  "vocabulary",
  "inference",
...
function statusForRatio(
  ratio: number,
): Pick<ReadingSkillInsight, "status" | "statusLabel" | "dots"> {
  ...
  for (const result of results) {
    ...
    if (primary) {
      for (const question of primary.questions) {
        ...
        if (isAnswerCorrect(question, result.primary.answers[question.id]))
          tallies[skill].correct += 1;
      }
    }
    ...
    for (const question of verification.questions) {
      const skill = skillByQuestionType[question.type];
      tallies[skill].total += 1;
      if (isAnswerCorrect(question, result.verification.answers[question.id]))
        tallies[skill].correct += 1;
    }
  }
      ...
      ? [
          {
            ...
            total: tally.total,
            ...statusForRatio(tally.correct / tally.total),
          },
        ]
      : [];
  });
