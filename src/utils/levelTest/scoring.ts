...
export function isAnswerCorrect(question: ReadingQuestion, answer: string | undefined) {
	if (!answer) return false;
	if (isGapFillQuestion(question)) {
		const accepted = question.acceptedAnswers?.length ? question.acceptedAnswers : [question.correctAnswer];
