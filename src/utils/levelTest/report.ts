...
import { createInitialAssessmentState, getCompletedLevelResults, submitPassage } from './engine';
...
export function isAnswerOptionId(value: unknown): value is AnswerOptionId {
	return typeof value === 'string' && (ANSWER_OPTION_IDS as readonly string[]).includes(value);
}

export function sanitizeAnswerSelection(value: unknown, questions: ReadingQuestion[]): AnswerSelection | null {
	if (!value || typeof value !== 'object' || Array.isArray(value)) return null;
	const answers: AnswerSelection = {};
	const record = value as Record<string, unknown>;
	for (const question of questions) {
		if (!(question.id in record) || record[question.id] == null || record[question.id] === '') continue;
...
export function parseReportAttempts(value: unknown): ReportAttemptInput[] | null {
	if (!Array.isArray(value) || value.length === 0 || value.length > CEFR_LEVELS.length) return null;
	...
	for (const entry of value) {
		...
		const primaryAnswers = sanitizeAnswerSelection(record.primaryAnswers, primary.questions);
		...
		if ('verificationAnswers' in record && record.verificationAnswers != null) {
			...
			const parsed = sanitizeAnswerSelection(record.verificationAnswers, verification.questions);
...
function answersForPassage(attempt: ReportAttemptInput | undefined, kind: ReadingPassageKind) {
	if (!attempt) return null;
	return kind === 'verification' ? attempt.verificationAnswers ?? null : attempt.primaryAnswers;
}

export function rebuildAssessmentFromAttempts(attempts: ReportAttemptInput[]): AssessmentState | null {
...
function buildQuestionRows(passage: ReadingPassage, answers: AnswerSelection): ReportQuestionRow[] {
...
function buildPassageSection(result: LevelResult, kind: ReadingPassageKind): ReportPassageSection | null {
...
export function buildReadingTestReport(state: AssessmentState): ReadingTestReport | null {
...
export function buildReportFromAttempts(attempts: ReportAttemptInput[], contentVersion: string) {
...
export function attemptsFromLevelResults(results: LevelResult[]): ReportAttemptInput[] {
	return results.map((result) => ({
		level: result.level,
		primaryAnswers: result.primary.answers,
		verificationAnswers: result.verification.attempted ? result.verification.answers : undefined,
	}));
}
