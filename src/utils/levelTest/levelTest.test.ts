...
describe('level-test content and scoring', () => {
	it('ships all 54 unique questions across A1-C2', () => {
		const questions = readingLevelContent.flatMap((entry) =>
			[...entry.primaryPassages, ...entry.verificationPassages].flatMap((passage) => passage.questions)
		);
		expect(questions).toHaveLength(54);
		expect(new Set(questions.map((question) => question.id)).size).toBe(54);
		expect(readingLevelContent.map((entry) => entry.level)).toEqual([...CEFR_LEVELS]);
	...
	it('keeps canonical form sizes and listening steps', () => {
		for (const level of CEFR_LEVELS) {
			...
			expect(getPassageForLevel(level, 'primary')?.questions).toHaveLength(questionsPerPrimaryPassage(level));
			expect(getPassageForLevel(level, 'verification')?.questions).toHaveLength(questionsPerVerificationPassage(level));
			expect(entry?.primaryPassages[0].format).toBe('reading');
			expect(entry?.primaryPassages.slice(1).every((step) => step.format === 'listening')).toBe(true);
		}
	});

	it('normalizes gap-fill answers', () => {
		const question = getPassageForLevel('A1')?.questions.find((item) => item.id === 'a1-r2');
...
describe('adaptive engine', () => {
	...
	it('uses verification before resolving a borderline level', () => {
		...
		state = recordPrimaryScore(state, { level: 'B1', passageId: 'b1', correct: 3, total: 6 });
		expect(state.currentPassageKind).toBe('verification');
		state = recordVerificationScore(state, { level: 'B1', passageId: 'b1-v', correct: 2, total: 3 });
...
describe('server report helpers', () => {
			...
			return Object.fromEntries(
				passage.questions.map((question, index) => [
					...
					index < correct
						...
						: question.options.find((option) => option.id !== question.correctAnswer)?.id ?? '__wrong__',
				])
			);
		};
	...
	it('serializes only answer attempts from completed results', () => {
		...
		expect(result && attemptsFromLevelResults([result])).toEqual([
			{ level: 'B1', primaryAnswers: { 'b1-r1': 'B' }, verificationAnswers: undefined },
		]);
	});
});
