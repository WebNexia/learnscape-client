...
import type { AnswerSelection, CefrLevel, ReadingPassage, ReadingPassageKind } from './types';
...
export type PrimaryClassification = 'confirmed' | 'needs_verification' | 'failed';
export type VerificationOutcome = 'passed' | 'failed';
export type LevelResolution = 'confirmed' | 'borderline' | 'failed' | 'pending_verification';
...
export type PrimaryResult = PassageScore & { classification: PrimaryClassification };
export type VerificationResult = { attempted: false } | (PassageScore & { attempted: true; passed: boolean });
...
export function classifyPrimaryResult(
	...
	if (total >= QUESTIONS_PER_UPPER_PASSAGE && (level === 'A1' || level === 'A2' || level === 'B1')) {
		if (correct >= 4) return 'confirmed';
		if (correct === 3) return 'needs_verification';
		return 'failed';
	}
	const confirmedMinimum =
		total >= QUESTIONS_PER_UPPER_PASSAGE ? UPPER_CONFIRMED_CORRECT_MINIMUM : CONFIRMED_CORRECT_MINIMUM;
	const borderlineExact =
		total >= QUESTIONS_PER_UPPER_PASSAGE ? UPPER_BORDERLINE_CORRECT_MINIMUM : BORDERLINE_CORRECT_MINIMUM;
	if (correct >= confirmedMinimum) return 'confirmed';
	if (correct === borderlineExact) return 'needs_verification';
	return 'failed';
}

export function evaluateVerification(correct: number, total = QUESTIONS_PER_VERIFICATION_PASSAGE): VerificationOutcome {
	const passMinimum = total >= 3 ? UPPER_VERIFICATION_PASS_MINIMUM : VERIFICATION_PASS_MINIMUM;
...
export function countCorrectAnswers(passage: ReadingPassage, answers: AnswerSelection) {
	return passage.questions.reduce(
		(total, question) => (isAnswerCorrect(question, answers[question.id]) ? total + 1 : total),
		0
	);
}
...
function planUpwardStep(result: LevelResult, highestConfirmedLevel: CefrLevel | null): NextStep {
	if (result.finalClassification === 'confirmed') {
		const nextLevel = getLevelAbove(result.level);
		if (!nextLevel) {
			return {
				kind: 'complete',
				outcome: { resultType: 'cefr', finalLevel: result.level, approachingLevel: null, status: 'confirmed' },
			};
		}
	...
	return {
		...
		outcome: {
			...
			approachingLevel: result.finalClassification === 'borderline' ? result.level : null,
			status: 'confirmed',
		},
	};
}
...
function planDownwardStep(
	...
	if (result.finalClassification === 'confirmed') {
		...
		const wasBorderlineAbove = Boolean(
			levelAbove && resultsByLevel[levelAbove]?.finalClassification === 'borderline'
		);
		return {
			kind: 'complete',
			outcome: {
	...
	if (nextLevel) return { kind: 'continue', level: nextLevel, passageKind: 'primary' };
	return {
		kind: 'complete',
		outcome: { resultType: 'below_a1', finalLevel: null, approachingLevel: null, status: null },
	};
}

function applyResolvedLevelResult(state: AssessmentState, result: LevelResult): AssessmentState {
	...
	const borderlineLevel =
		result.finalClassification === 'borderline' ? result.level : state.borderlineLevel;
	const direction: AssessmentDirection =
		state.direction ?? (result.finalClassification === 'confirmed' ? 'up' : 'down');
	const nextStep =
		direction === 'up'
	...
	return {
		...
		currentPassageKind: nextStep.kind === 'continue' ? nextStep.passageKind : null,
...
export function recordPrimaryScore(
	state: AssessmentState,
	{
		...
		total = QUESTIONS_PER_PASSAGE,
		answers = {},
	}: { level: CefrLevel; passageId: string; correct: number; total?: number; answers?: AnswerSelection }
) {
	const primary: PrimaryResult = {
		...createPassageScore({ passageId, kind: 'primary', correct, total, answers }),
...
export function recordVerificationScore(
	...
	const verification: VerificationResult = {
		attempted: true,
		...createPassageScore({ passageId, kind: 'verification', correct, total, answers }),
	...
	return applyResolvedLevelResult(state, {
		...
		primary,
		verification,
		finalClassification: resolveLevelClassification(primary.classification, verificationOutcome),
	});
}

export function submitPassage(state: AssessmentState, passage: ReadingPassage, answers: AnswerSelection) {
	const payload = {
		level: passage.level,
...
export function getCompletedLevelResults(state: AssessmentState): LevelResult[] {
...
export function summarizeAssessment(state: AssessmentState) {
	...
	const numberOfVerificationPassages = results.filter((result) => result.verification.attempted).length;
	return {
