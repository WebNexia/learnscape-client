...
const AdminRecycleBinDocumentsTab = () => {
  ...
  const fetchMoreContextData = useCallback(
    ...
    [fetchArchivedDocuments, loadedPages],
  );
  ...
  const handleSelectAll = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (checked) {
      setSelectedItems(
        paginatedDocuments?.map((document) => document._id) || [],
      );
      setSelectAll(true);
    } else {
      setSelectedItems([]);
  ...
  const restoreDocument = async (documentId: string) => {
    ...
    try {
      const response = await axios.patch(
        `${base_url}/documents/${documentId}/restore`,
      );
  ...
  const hardDeleteDocument = async (documentId: string) => {
    ...
    try {
      const response = await axios.delete(
        `${base_url}/documents/${documentId}/hard`,
      );
  ...
  const handleBulkRestore = async () => {
    try {
      ...
      setArchivedDocuments(
        (prev) =>
          prev?.filter((document) => !selectedItems?.includes(document._id)) ||
          [],
      );
      ...
      setSnackbarMessage(
        `${selectedItems.length} document(s) restored successfully`,
      );
      setSnackbarSeverity("success");
      setSnackbarOpen(true);
    } catch (error) {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      await Promise.all(
        selectedItems?.map((documentId) =>
          axios.delete(`${base_url}/documents/${documentId}/hard`),
        ) || [],
      );
      ...
      setSnackbarMessage(
        `${selectedItems.length} document(s) permanently deleted`,
      );
      setSnackbarSeverity("success");
      setSnackbarOpen(true);
    } catch (error) {
    ...
    <>
      ...
      >
        ...
        >
          ...
          <TableBody>
            {paginatedDocuments &&
              paginatedDocuments?.map((document: ArchivedDocument) => {
                  ...
                  <TableRow key={document._id} hover selected={isSelected}>
                    <TableCell padding="checkbox" sx={{ textAlign: "center" }}>
                      <input
                        type="checkbox"
                        checked={isSelected}
                        onChange={() => handleSelectItem(document._id)}
                      />
                    </TableCell>
                    <CustomTableCell
                      value={truncateText(
                    ...
                    {!isMobileSize && (
                      <CustomTableCell
                        value={document.archivedByName || "N/A"}
                      />
                    )}
                    ...
                    <CustomTableCell
                      value={
                        document.archivedAt
                          ? dateFormatter(document.archivedAt)
                          : "N/A"
                      }
                    />
                    {!isMobileSize && (
                    ...
                    <TableCell sx={{ textAlign: "center" }}>
                      <CustomActionBtn
                        ...
                        onClick={() => openItemDialog("restore", document)}
                        icon={
                          <Restore
                            fontSize="small"
                            sx={{
                      ...
                      <CustomActionBtn
                        ...
                        onClick={() => openItemDialog("delete", document)}
                        icon={
                          <DeleteForever
                            fontSize="small"
        ...
        {paginatedDocuments && paginatedDocuments.length === 0 && (
          <CustomInfoMessageAlignedLeft
            message={
              isSearchActive
                ? "No deleted documents found matching your search criteria."
      ...
      {/* Restore Modal */}
      {activeItemDialog === "restore" && selectedDocument && (
        <CustomDialog
          openModal={true}
        ...
        >
          <DialogContent>
            ...
            >
              ...
              {truncateText(selectedDocument.name || "", 25)}"?
            </Typography>
            <Typography
              variant="body2"
              sx={{
          ...
          <CustomDialogActions
            ...
            onSubmit={() => restoreDocument(selectedDocument._id)}
      ...
      {/* Delete Modal */}
      {activeItemDialog === "delete" && selectedDocument && (
        <CustomDialog
          openModal={true}
          closeModal={closeItemDialog}
        ...
        >
          ...
          <CustomDialogActions
            onCancel={closeItemDialog}
            onDelete={() => hardDeleteDocument(selectedDocument._id)}
            deleteBtn={true}
      ...
      {isInfoDialogOpen && (
        ...
        >
          <DialogContent>
            ...
            >
              Documents in the recycle bin are automatically permanently deleted
            ...
            >
              You can restore documents before this date or manually delete them
