...
const AdminRecycleBinQuestionsTab = () => {
  ...
  const { addNewQuestion, questionTypes, fetchQuestionTypeName } =
  ...
  const fetchMoreContextData = useCallback(
    ...
    [fetchArchivedQuestions, loadedPages],
  );
  ...
  const [selectedQuestion, setSelectedQuestion] =
    useState<ArchivedQuestion | null>(null);
  const [activeItemDialog, setActiveItemDialog] = useState<
    "restore" | "delete" | null
  >(null);
  const [isBulkRestoreModalOpen, setIsBulkRestoreModalOpen] =
    useState<boolean>(false);
  const [isBulkDeleteModalOpen, setIsBulkDeleteModalOpen] =
    useState<boolean>(false);
  const [isItemMutating, setIsItemMutating] = useState(false);
  ...
  const handleSelectAll = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (checked) {
      setSelectedItems(
        paginatedQuestions?.map((question) => question._id) || [],
      );
      setSelectAll(true);
    } else {
      setSelectedItems([]);
  ...
  const openItemDialog = (
    dialog: "restore" | "delete",
    question: ArchivedQuestion,
  ) => {
    setSelectedQuestion(question);
    setActiveItemDialog(dialog);
  };

  const closeItemDialog = () => {
    if (isItemMutating) return;
  ...
  const restoreQuestion = async (questionId: string) => {
    if (isItemMutating) return;
    setIsItemMutating(true);
    try {
      const response = await axios.patch(
        `${base_url}/questions/${questionId}/restore`,
      );
      ...
      if (response.data.status === 200) {
        ...
        if (response.data.data) {
          addNewQuestion({
            ...response.data.data,
            questionType: fetchQuestionTypeName(response.data.data),
          });
          setSelectAll(false);
          setSelectedItems([]);
        }
  ...
  const hardDeleteQuestion = async (questionId: string) => {
    ...
    try {
      const response = await axios.delete(
        `${base_url}/questions/${questionId}/hard`,
      );
  ...
  const handleBulkRestore = async () => {
    try {
      ...
      setSnackbarMessage(
        `${selectedItems.length} question(s) restored successfully`,
      );
      setSnackbarSeverity("success");
      setSnackbarOpen(true);
    } catch (error) {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      await Promise.all(
        selectedItems?.map((questionId) =>
          axios.delete(`${base_url}/questions/${questionId}/hard`),
        ) || [],
      );
      ...
      setSnackbarMessage(
        `${selectedItems.length} question(s) permanently deleted`,
      );
      setSnackbarSeverity("success");
      setSnackbarOpen(true);
    } catch (error) {
  ...
  const getDeletionDateStatus = (archivedAt: string) => {
    ...
    const deletionDate = new Date(
      archivedDate.getTime() + 7 * 24 * 60 * 60 * 1000,
    ); // 7 days
    const now = new Date();
    ...
    if (daysUntilDeletion <= 0) {
      return {
        label: `${dateFormatter(deletionDate)}`,
    ...
    <>
      ...
      >
        ...
        >
          ...
          <TableBody>
            {paginatedQuestions &&
              paginatedQuestions?.map((question: ArchivedQuestion) => {
                const deletionDateStatus = getDeletionDateStatus(
                  question.archivedAt || "",
                );
                  ...
                  <TableRow key={question._id} hover selected={isSelected}>
                    <TableCell padding="checkbox" sx={{ textAlign: "center" }}>
                      <input
                        type="checkbox"
                        checked={isSelected}
                        onChange={() => handleSelectItem(question._id)}
                      />
                    </TableCell>
                    <CustomTableCell
                      value={
                        isMobileSize
                          ? truncateText(
                              stripHtml(decode(question.question)),
                              25,
                            )
                          : truncateText(
                              stripHtml(decode(question.question)),
                    ...
                    {!isMobileSize && (
                      <CustomTableCell
                        value={question.questionTypeName || "N/A"}
                      />
                    )}
                    {!isMobileSize && (
                      <CustomTableCell
                        value={question.archivedByName || "N/A"}
                      />
                    )}
                    <CustomTableCell
                      value={
                        question.archivedAt
                          ? dateFormatter(question.archivedAt)
                          : "N/A"
                      }
                    />
                    {!isMobileSize && (
                      <CustomTableCell value={deletionDateStatus.label} />
                    )}
                    <TableCell sx={{ textAlign: "center" }}>
                      <CustomActionBtn
                        title="Restore Question"
                        onClick={() => openItemDialog("restore", question)}
                        icon={
                          <Restore
                            fontSize="small"
                            sx={{
                              mr: isMobileSize ? "0rem" : "-0.6rem",
                      ...
                      <CustomActionBtn
                        ...
                        onClick={() => openItemDialog("delete", question)}
                        icon={
                          <DeleteForever
                            fontSize="small"
                            sx={{ fontSize: isMobileSize ? "1rem" : undefined }}
                          />
                        }
                      />
                    </TableCell>
        ...
        {paginatedQuestions && paginatedQuestions.length === 0 && (
          <CustomInfoMessageAlignedLeft
            message={
              isSearchActive
                ? "No deleted questions found matching your search criteria."
                : "No deleted questions found."
            }
            sx={{
              marginTop: isMobileSize ? "3rem" : "5rem",
              marginBottom: "1rem",
            }}
          />
        )}
      ...
      {/* Restore Modal */}
      {activeItemDialog === "restore" && selectedQuestion && (
        <CustomDialog
          openModal={true}
        ...
        >
          <DialogContent>
            ...
            >
              ...
              {truncateText(stripHtml(decode(selectedQuestion.question)), 25)}"?
            </Typography>
            <Typography
              variant="body2"
              sx={{
          ...
          <CustomDialogActions
            ...
            onSubmit={() => restoreQuestion(selectedQuestion._id)}
            submitBtnText="Restore"
            isSubmitting={isItemMutating}
            disableBtn={isItemMutating}
            disableCancelBtn={isItemMutating}
      ...
      {/* Delete Modal */}
      {activeItemDialog === "delete" && selectedQuestion && (
        <CustomDialog
          openModal={true}
          closeModal={closeItemDialog}
        ...
        >
          ...
          <CustomDialogActions
            onCancel={closeItemDialog}
            onDelete={() => hardDeleteQuestion(selectedQuestion._id)}
            deleteBtn={true}
            deleteBtnText="Delete Permanently"
            isDeleting={isItemMutating}
            disableBtn={isItemMutating}
            disableCancelBtn={isItemMutating}
            actionSx={{ marginBottom: "0.5rem" }}
          />
      ...
      {isBulkRestoreModalOpen && (
        ...
        >
          <DialogContent>
            ...
            >
              Are you sure you want to restore {selectedItems.length} selected
              question(s)?
            </Typography>
            <Typography
              variant="body2"
      ...
      {isInfoDialogOpen && (
        <CustomDialog
          ...
          title="Auto-Removal Information"
          maxWidth="sm"
        >
          <DialogContent>
            <Typography
            ...
            >
              Questions in the recycle bin are automatically permanently deleted
            ...
            >
              You can restore questions before this date or manually delete them
              immediately using the "Delete Permanently" button.
            </Typography>
          </DialogContent>
          <CustomDialogActions
