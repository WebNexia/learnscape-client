import { Avatar, Box, CircularProgress, DialogActions, DialogContent, Grid, Typography } from '@mui/material';
...
const CoursesInfoModal = ({ courseId, courseTitle, isCourseInfoDialogOpen, setIsCourseInfoDialogOpen }: CoursesInfoModalProps) => {
	...
	const [courseInfo, setCourseInfo] = useState<Partial<SingleCourse> | null>(null);
	const [loading, setLoading] = useState(false);
	const [fetchError, setFetchError] = useState<string | null>(null);

	useEffect(() => {
		if (!isCourseInfoDialogOpen || !courseId) {
			setCourseInfo(null);
			setFetchError(null);
			return;
		}
		...
		const fetchStaffInfo = async () => {
			...
			try {
				...
				if (!cancelled) {
					setFetchError('Failed to load course info');
					setCourseInfo(null);
				}
			} finally {
				if (!cancelled) {
	...
	return (
		<CustomDialog
			...
			title={singleCourse?.title || courseTitle}
			maxWidth='sm'
			...
			<DialogContent>
				{loading ? (
					...
					<Typography variant='body2' color='error' sx={{ fontSize: isMobileSize ? '0.75rem' : '0.85rem' }}>
						{fetchError}
					</Typography>
				) : (
