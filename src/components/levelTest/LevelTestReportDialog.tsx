...
const errorCopy: Record<string, string> = {
  ...
  rate_limited:
    "Kısa sürede çok fazla rapor istendi. Lütfen biraz sonra yeniden dene.",
  service_unavailable:
    "Rapor servisi şu anda kullanılamıyor. Lütfen daha sonra yeniden dene.",
  ...
  const send = async () => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_SERVER_BASE_URL}/level-test/report`,
        {
          method: "POST",
          ...
          (response.status === 429
            ...
            : response.status === 503
              ...
              : response.status === 400 &&
                ...
                : "send_failed");
        throw new Error(
          payload?.message || errorCopy[code] || errorCopy.send_failed,
        );
      }
      setStatus("sent");
    } catch (requestError) {
      ...
      <DialogContent sx={{ pt: 3 }}>
        ...
        {status === "sent" ? (
          <Alert severity="success">
            Rapor başarıyla gönderildi. Gelen kutunu ve gerekirse spam klasörünü
            kontrol et.
          </Alert>
        ) : (
          <Box
      ...
      <DialogActions sx={{ px: 3, pb: 2.5 }}>
        ...
        {status !== "sent" ? (
          <Button
            ...
            variant="contained"
            disabled={
              status === "sending" || (turnstileEnabled && !recaptchaToken)
            }
            sx={primaryButtonSx}
          >
