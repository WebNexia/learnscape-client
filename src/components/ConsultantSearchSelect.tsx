...
/**
 * Search and select users with role admin, owner, super-admin, instructor, or consultant.
 * Used for consultation slots and other flows that need to pick consultants / org staff.
 * Kept separate from EventUserSearchSelect (which targets learners) for clearer ownership and future changes.
 */
...
const ConsultantSearchSelect = forwardRef<any, ConsultantSearchSelectProps>(
	(
		{
			value,
			onChange,
			onSelect,
			currentUserId,
			placeholder = 'Search consultants / admins...',
			sx = {},
			listSx = {},
			disabled = false,
			selectedUserIds = [],
		...
		const {
			...
			userRole: 'admin', // admin, owner, super-admin, instructor, consultant (backend includes consultant for future)
		});
						...
						InputProps={{
							onKeyDown: (e) => {
								if (e.key === 'Enter') {
									e.preventDefault();
									e.stopPropagation();
									if (value.trim() && !disabled && !loading) handleSearch();
								}
							},
							endAdornment: (
					...
					!loading && (
						<Box
							...
							<CustomErrorMessage sx={{ fontSize: isMobileSize ? '0.65rem' : '0.75rem' }}>
								No consultants or admins found matching your search.
							</CustomErrorMessage>
						</Box>
					)
				)}
...
ConsultantSearchSelect.displayName = 'ConsultantSearchSelect';

export default ConsultantSearchSelect;
