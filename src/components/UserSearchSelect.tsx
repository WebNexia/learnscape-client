...
const UserSearchSelect: React.FC<UserSearchSelectProps> = ({
  ...
  return (
    <Box sx={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', mb: filtered.length === 0 ? '1.5rem' : '-1rem', margin: `0 auto ${filtered.length === 0 ? '1.5rem' : '-1rem'} auto`,  }}>
      <CustomTextField
        sx={sx}
        value={value}
